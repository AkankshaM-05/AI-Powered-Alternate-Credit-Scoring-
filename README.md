...
In the notebook, we implemented a complete ML pipeline using Python (3.10+) and publicly available, anonymized datasets such as German Credit (Statlog-Kaggle). We performed data cleaning, preprocessing, and feature engineering using pandas and numpy. We engineered behavioral financial ratios such as Repayment Burden, Liquidity Ratio, and a Risk Multiplier to better capture real-time repayment capacity.
